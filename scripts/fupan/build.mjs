// 复盘故事视频 · 画面生成器
// 读 remotion/src/fupan/generated/timing.json(配音句级时间轴) → 生成 Remotion 用的
//   generated/markup.ts   场景 DOM(人物 SVG / 图表 / 字幕逐字 span)
//   generated/styles.ts   作用域化 CSS(.fp-scope 前缀, 不污染 Remotion Studio)
//   generated/timeline.ts GSAP 编排(按句/按词触发, FupanStory 每帧 seek)
// 用法: node build.mjs   (run.sh 会自动调用)
...
`;
// ---------- emit Remotion modules ----------
function scope(css) {
  css = css.replace(/\/\*[\s\S]*?\*\//g, "").replace(/@font-face\s*\{[^}]*\}/g, "");
  return css.replace(/([^{}]+)\{/g, (m, sel) => {
    sel = sel.trim();
    if (sel.startsWith("@")) return m;
    return sel.split(",").map((x) => {
      x = x.trim();
      if (x === "html" || x === "body" || x === ":root") return ".fp-scope";
      if (x === "*") return ".fp-scope, .fp-scope *";
      return ".fp-scope " + x.replace(/^#root\b/, "#fp-root");
    }).join(", ") + " {";
  });
}
const css = scope(html.match(/<style>([\s\S]*?)<\/style>/)[1]);
let markup = html.slice(html.indexOf("<body>") + 6, html.lastIndexOf("<script>"));
...
fs.writeFileSync(path.join(GEN, "styles.ts"), banner + "export const styles = " + JSON.stringify(css) + ";\n");
fs.writeFileSync(path.join(GEN, "timeline.ts"), banner + "// @ts-nocheck\nexport function buildTimeline(gsap) {\n" + tlSrc + "\n}\n");
fs.writeFileSync(path.join(GEN, "meta.ts"), banner + `export const FUPAN_DURATION_SEC = ${D};\n`);
fs.mkdirSync(WORK, { recursive: true });
fs.writeFileSync(path.join(WORK, "chars.txt"), html.replace(/<script[\s\S]*?<\/script>/g, "") + T.phrases.map((p) => p.text).join(""));
console.log("built", GEN, "duration", D);
