...
# ---------- master ----------
full=V*1.0+M+X
g=0.92/np.abs(full).max()
fade=np.minimum(1,(D-t)/0.4)
bed=(M+X)*fade*g; voice=V*g
sf.write(sys.argv[3],np.stack([voice,voice],1).astype(np.float32),SR,subtype='PCM_16')
sf.write(sys.argv[4],np.stack([bed,bed],1).astype(np.float32),SR,subtype='PCM_16')
print('ok',round(D,2))
